...
const Goals = () => {
  ...
  const progressFillStyle = (percentage) => ({
    height: '100%',
    background: percentage >= 100 ? '#28a745' : percentage >= 75 ? '#ffc107' : '#007bff',
    width: `${percentage}%`,
                ...
                <div key={goal.id} style={goalCardStyle}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                    <div>
                      ...
                      <span style={{
                        background: isCompleted ? '#28a745' : isOverdue ? '#dc3545' : '#007bff',
                        color: 'white',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                    <div>
                      <p style={{ margin: '0.25rem 0', color: '#495057', fontSize: '0.9rem' }}>
                        📅 Target: {format(parseISO(goal.targetDate), 'MMM dd, yyyy')}
                      </p>
                      <p style={{ 
                        margin: '0.25rem 0', 
                        color: isOverdue ? '#dc3545' : daysRemaining <= 7 ? '#ffc107' : '#495057',
                        fontSize: '0.9rem',
                        fontWeight: isOverdue || daysRemaining <= 7 ? '600' : 'normal'
                      }}>
                        {isCompleted ? '🎉 Goal Achieved!' : 
                         isOverdue ? `⚠️ ${Math.abs(daysRemaining)} days overdue` :
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      onClick={() => {
                        const newProgress = prompt(`Update progress for "${goal.title}" (current: ${goal.progress} ${goal.unit}):`);
                        if (newProgress !== null && !isNaN(newProgress)) {
                          storageService.updateGoal(goal.id, { ...goal, progress: parseFloat(newProgress) });
                          setGoals(storageService.getGoals());
                        }
                      }}
